...
\n\n-- Drop existing policies if they exist\nDROP POLICY IF EXISTS "Users can request verification" ON profiles;
\nDROP POLICY IF EXISTS "Admins can manage verification status" ON profiles;
\n\n-- Create simplified policies for verification\nCREATE POLICY "verification_request_policy"\n  ON profiles FOR UPDATE\n  TO authenticated\n  USING (auth.uid() = id)\n  WITH CHECK (\n    auth.uid() = id AND\n    verification_status = 'pending'\n  );
\n\nCREATE POLICY "admin_verification_policy"\n  ON profiles FOR UPDATE\n  TO authenticated\n  USING (auth.email() = 'asassin.damian@gmail.com')\n  WITH CHECK (auth.email() = 'asassin.damian@gmail.com');
;
